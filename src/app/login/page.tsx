...
function LoginForm() {
  ...
  async function handleEmailLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (authError) {
        // Friendlier rate limit message
        if (authError.message?.toLowerCase().includes('rate limit')) {
          throw new Error('Too many sign-in attempts. Please try signing in with Google instead, or wait a few minutes before trying again.');
        }
        throw authError;
      }
      setSent(true);
      ...
      <main className="flex-1 flex items-center justify-center px-4">
        <div className="bg-surface border border-border rounded-xl p-8 max-w-md w-full space-y-6">
          <div className="text-center">
            ...
            <p className="text-muted text-sm mt-1">Sign in to pick your boxes</p>
          </div>

          {error && (
            <p className="text-ne-red text-sm text-center bg-ne-red/10 border border-ne-red/20 rounded-lg p-3">{error}</p>
          )}
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 bg-white text-gray-800 font-semibold py-2.5 rounded-lg hover:bg-gray-100 transition disabled:opacity-50"
          >
          ...
          <p className="text-xs text-muted text-center">
            No password needed — sign in instantly with Google.
          </p>
        </div>
      </main>
    </div>
